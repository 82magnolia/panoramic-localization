...
def localize(cfg: NamedTuple, log_dir: str):
    ...
    for trial, filename in enumerate(filenames):
        ...
        # Log error metrics depending on refine_mode
        ...
        if print_retrieval:
            print("best t-error: ", (trans_tensors - gt_trans.T).norm(dim=-1).min().item())
            print("best r-error: ", r_error.min().item())
            print("retrieved t-error: ", (trimmed_trans - gt_trans.T).norm(dim=-1).min().item())
            print("retrieved r-error: ", rot_k_error.min().item())
